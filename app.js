...
function renderTodayCard(day) {
  ...
  const fujiSilhouette = `<svg style="position:absolute;right:-32px;bottom:0;opacity:0.18;pointer-events:none" width="320" height="240" viewBox="0 0 800 340"  xmlns="http://www.w3.org/2000/svg"><path d="M0 340 L100 295 L195 245 L290 185 L355 130 L388 92 L400 68 L412 92 L445 130 L510 185 L605 245 L700 295 L800 340 Z" fill="currentColor"/><path d="M376 124 L389 90 L400 68 L411 90 L424 124 L418 136 L408 144 L400 146 L392 144 L382 136 Z" fill="white" opacity="0.6"/></svg>`;
  ...
  const northCard = `
    <div style="position:relative;overflow:hidden;border-radius:1.5rem;padding:1.5rem 1.5rem 0;display:flex;flex-direction:column;min-height:220px;background:${nt.bg}">
      ...
      <h3 style="position:relative;z-index:10;margin-top:auto;margin-bottom:32px" class="font-headline text-2xl font-bold ${nt.text}">${ns.desc}</h3>
  ...
  const southCard = `
    <div style="position:relative;overflow:hidden;border-radius:1.5rem;padding:1.5rem 1.5rem 0;display:flex;flex-direction:column;min-height:220px;background:${st.bg}">
      ...
      <h3 style="margin-top:auto;margin-bottom:32px" class="font-headline text-2xl font-bold ${st.text}">${ss.desc}</h3>
